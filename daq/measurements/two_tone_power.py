# -*- coding: utf-8 -*-
"""
Two-tone spectroscopy in Lockin mode: 2D sweep of pump power and frequency,
with fixed probe.
Modified from https://github.com/intermod-pro/presto-measure/blob/master/
two_tone_power.py
"""
...
class TwoTonePower(Base):
    ...
    def run(
        ...
        with lockin.Lockin(
            ...
            **self.DC_PARAMS,
            ...
            f_start = (
                self.control_freq_center - self.control_freq_span / 2
            )
            f_stop = (
                self.control_freq_center + self.control_freq_span / 2
            )
            ...
            for jj, control_amp in enumerate(self.control_amp_arr):
                ...
                for ii, control_freq in enumerate(self.control_freq_arr):
                    ...
                    _d = lck.get_pixels(
                        self.num_skip + self.num_averages, quiet=True
                    )
                    data_i = _d[self.input_port][1][:, 0]
                    data_q = _d[self.input_port][2][:, 0]
                    data = (
                        data_i.real + 1j * data_q.real
                    )  # using zero IF

                    self.resp_arr[jj, ii] = np.mean(
    ...
    def load(cls, load_filename: str) -> "TwoTonePower":
        with h5py.File(load_filename, "r") as h5f:
            ...
            control_freq_center = float(
                h5f.attrs["control_freq_center"]
            )  # type: ignore
            control_freq_span = float(
                h5f.attrs["control_freq_span"]
            )  # type: ignore
            ...
            control_amp_arr: npt.NDArray[np.float64] = h5f[
                "control_amp_arr"
            ][()]  # type: ignore
            control_freq_arr: npt.NDArray[np.float64] = h5f[
                "control_freq_arr"
            ][()]  # type: ignore
            resp_arr: npt.NDArray[np.complex128] = h5f["resp_arr"][
                ()
            ]  # type: ignore
    ...
    def analyze(
        ...
        if linecut and blit:
            ...
            ax2.draw_artist(line_a)  # pyright: ignore
            fig1.canvas.blit(fig1.bbox)
