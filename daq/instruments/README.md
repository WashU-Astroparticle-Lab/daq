...
> wrote. For a short pulse at a slow rate, use [pulse mode](#pulse-mode-explicit-width) instead.

### Pulse mode (explicit width)

Takes the pulse **width** directly rather than a duty cycle, so width and repetition rate are
independent. This is the mode for a narrow pulse at a slow rate — the combination PWM cannot
reach.

```python
with DC2200() as led:
    led.configure_pulse(current_a=1.0e-3, width_s=10e-6, freq_hz=2.0, count=25)
...
| Argument | Value | Meaning |
|---|---|---|
| `current_a` | 1.0e-3 | Drive the LED at **1 mA** during each pulse |
| `width_s` | 10e-6 | Each pulse is **10 µs** long |
| `freq_hz` | 2.0 | Repeat every **500 ms** (give `period_s=0.5` instead if you prefer) |
| `count` | 25 | Emit **25 pulses**, then stop |

So 25 pulses of 10 µs spaced 500 ms apart — a 12.5 s train carrying 250 µs of total LED-on
time. Give the rate as either `freq_hz` or `period_s`, exactly one of the two.

Like PWM, the train starts when the output is enabled, which is a software-timed USB write, so
it is **not synchronised** to an acquisition. Enabling the output returns immediately, so start
the train and then call `run()`:

```python
led.configure_pulse(current_a=1.0e-3, width_s=10e-6, freq_hz=2.0,
                    count=int(np.ceil(duration_s * 2.0)) + 5)   # outlast the record
ts.attach(led=led)
led.output = True
try:
    ts.run()
...
`settings()` reports `mode`, `pulse_current_a`, `pulse_width_s`, `pulse_count`, and both
`pulse_freq_hz` and `pulse_period_s` regardless of which form the instrument uses internally —
so `attach()` records `led_pulse_width_s`, `led_pulse_freq_hz` and the rest.

Validation rejects a width that is not shorter than the period (the LED would never switch
off), a non-integer or non-positive `count`, and a current above the queried limit.

> Thorlabs does not publish the SCPI headers for pulse mode, so the driver resolves each one
> against the instrument on first use and caches it — see the gotchas table below. Inspect
> `led._resolved_headers` after the first call to see what your firmware accepted.
...
| DC2200 | Pulse mode's headers are likewise unpublished, and are resolved the same way from `PULSE_MODE_COMMANDS`, `PULSE_CURRENT_HEADERS`, `PULSE_WIDTH_HEADERS`, `PULSE_PERIOD_HEADERS` / `PULSE_FREQ_HEADERS` and `PULSE_COUNT_HEADERS`. The period spellings are tried before the frequency ones, since the two take different units. A setting whose candidates are all rejected raises an error naming the exact attribute to extend. |
