...
- **`DC2200`** (`dc2200.py`) — Thorlabs LED driver. `configure_cc()`, `configure_pwm()`, `pulse_train()` (configure → on → wait → off, off guaranteed), `configure_ttl()`, `measured_current`. Currents are validated against the instrument's own queried `SOURCE1:CURRENT:LIMIT?`. Preserves the two required Thorlabs spellings: `SOURCE1:CCURENT:CURRENT` (their misspelling) and `SOURCE1:PWM:FREQ` (the long form `FREQUENCY` is rejected). `configure_ttl()` puts the LED under the rear-panel SMA modulation input (0–5 V, ≤250 kHz) so the Presto trigger gates it — this is how an LED pulse is synchronised to a `TimeStream` (`external_trigger=True`, and **`discard_start_ms=0`**, since the default 25 ms trim would swallow all but 5 ms of the 30 ms trigger window). Thorlabs does not publish the SCPI header for the TTL-mode current, so `TTL_CURRENT_HEADERS` lists candidates that `configure_ttl()` tries against the instrument, keeping whichever is accepted rather than hardcoding a guess.
