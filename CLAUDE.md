...
- **`DC2200`** (`dc2200.py`) — Thorlabs LED driver, four modes distinguished by what generates the pulse timing. `configure_cc()` (steady), `configure_pwm()` / `pulse_train()` (duty-cycle-defined burst; `pulse_train` is the blocking configure → on → wait → off form, `configure_pwm` the non-blocking one), `configure_pulse()` (**width**-defined burst), `configure_ttl()` (follows an external signal), plus `measured_current`. The PWM **duty floor is 0.1 %** and smaller requests are silently coerced up, so PWM's narrowest pulse is `0.001 / freq_hz` — 10 µs only at 100 Hz, 500 µs at 2 Hz. `configure_pulse(current_a, width_s, freq_hz=|period_s=, count=)` sets the width explicitly and so decouples width from rate, which is the only way to get a short pulse at a slow repetition rate; it rejects a width that is not shorter than the period. Neither PWM nor pulse mode is synchronised to an acquisition (the train starts on a software write); only `configure_ttl()` is. Currents are validated against the instrument's own queried `SOURCE1:CURRENT:LIMIT?`. Preserves the two required Thorlabs spellings: `SOURCE1:CCURENT:CURRENT` (their misspelling) and `SOURCE1:PWM:FREQ` (the long form `FREQUENCY` is rejected). `configure_ttl()` puts the LED under the rear-panel SMA modulation input (0–5 V, ≤250 kHz) so the Presto trigger gates it — "TTL" is the plain digital-logic sense (transistor–transistor logic: 0 V low, ~5 V high), and this is how illumination is synchronised to a `TimeStream`. The LED is wired to **digital output port 2** in the lab's default setup, so pass `external_trigger=[0, 1]` (`True` would gate port 1, the 33220A, and leave the LED dark); use `[1, 1]` to fire both. Also set **`discard_start_ms=0`**, since the LED comes up with the RF drive at `t = 0` and the default 25 ms trim would drop the turn-on transient. Note this yields an illumination *window* spanning the acquisition, not a pulse within it (see the `TimeStream` entry) — a short synchronised flash needs `presto.pulsed`, while `configure_pwm`/`configure_pulse` give real short pulses at an unknown offset. Configuring is *not* arming: like `configure_cc`/`configure_pwm`, it defaults to `output=False` so it can never illuminate the LED as a side effect; enabling the output arms the stage, after which the LED follows the modulation input. Arm immediately before `run()` — the idle level of the Presto trigger line between acquisitions is not controlled here and has not been measured, so an LED armed during setup may sit lit through the whole connect/configure/tune phase. Thorlabs does not publish the SCPI headers for the TTL or pulse modes, so the driver keeps candidate lists (`TTL_CURRENT_HEADERS`, `PULSE_*_HEADERS`) and resolves each against the instrument on first use via `_write_first_accepted()`, caching the accepted spelling in `_resolved_headers`. A rejected header queues `-113 Undefined header`, which `write()` raises, so this resolves deterministically instead of hardcoding a guess; if every candidate for a setting fails, the error names the exact class attribute to extend. Inspect `led._resolved_headers` to see what a given firmware accepted.
