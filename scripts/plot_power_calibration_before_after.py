...
old = np.load(
    io.BytesIO(
        subprocess.check_output(
            ["git", "show", "origin/main:daq/calibrations/power_calibration.npz"]
        )
    )
)
...
for ax, a32, txt in (
    (ax0, old_amp_32, f"−32 dBm → amp {old_amp_32:.4f}"),
    (ax1, new_amp_32, f"−32 dBm → amp {new_amp_32:.4f}"),
):
    ax.plot([a32], [-32], marker="X", markersize=13, color="tab:red", zorder=6)
    ax.annotate(
        txt, (a32, -32), xytext=(-150, 14), textcoords="offset points", color="tab:red", fontsize=11
    )
...
extra = [
    Line2D([], [], color="0.3", linewidth=2.2, label="verified region"),
    Line2D(
        [],
        [],
        color="0.3",
        linewidth=1.6,
        linestyle=(0, (3, 2)),
        label="extrapolated (CalibrationWarning)",
    ),
    Line2D(
        [],
        [],
...
ax1.legend(
    handles=ax1.get_legend_handles_labels()[0] + extra, fontsize=9, ncol=2, loc="lower right"
)
fig.suptitle(
    "Power calibration before and after the fix\n"
    "dots = spectrum-analyzer measurements; lines = packaged calibration; gray = below the 0.4 fit cutoff",
    fontsize=13,
)
fig.savefig(OUT, dpi=170)
print("wrote", OUT, "old amp", old_amp_32, "new amp", new_amp_32)
